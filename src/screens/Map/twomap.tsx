...
const TwoMapScreen = (props: any) => {
  ...
  const [ShowProfileForGtoGMatching, setShowProfileForGtoGMatching] =
    useState(false);
  ...
  function UpdateMyLocationForGtoG() {
    ...
    Geolocation.getCurrentPosition(
      (position) => {
        ...
        reference.ref(`/GtoGLocations/${ReplaceUserEmail}`).update({
          latitude: latitude,
          longitude: longitude,
  ...
  useEffect(() => {
    if (GpsOn == true) {
      let Interval = setInterval(UpdateMyLocationForGtoG, 20000);
      return () => {
  ...
  const {
    data,
    isLoading,
    refetch: GtoGLocationsRefetch,
  } = useQuery('GtoGLocations', Get_GtoGLocations);
  const {data: itaewon_HotPlaceList, isLoading: itaewon_HotPlaceListisLoading} =
    useQuery('itaewon_HotPlaceList2', Get_itaewon_HotPlaceList);
  ...
  const StartChatingBetweenGirls = () => {
    ...
    {
      isEmptyObj(ProfileForGtoG)
        ? null
        : params.addUserIds([
            `${ProfileForGtoG.UserEmail}`,
            `${UserData.UserEmail}`,
          ]);
      ...
      <Modal
        ...
        <SafeAreaView style={MapScreenStyles.Memomodal}>
          ...
          <View style={[styles.W90ML5]}>
            ...
            />
            <Text style={MapScreenStyles.WhiteText}>
              HB:{ProfileForGtoG?.Grade}점
            </Text>
            <Text style={MapScreenStyles.WhiteText}>
              Tesnion:{ProfileForGtoG?.TensionGrade}점
            </Text>
            <Text style={MapScreenStyles.WhiteText}>
              사회성:{ProfileForGtoG?.SocialGrade}점
            </Text>
            <Text style={MapScreenStyles.WhiteText}>
              매너:{ProfileForGtoG?.MannerGrade}점
            </Text>
          </View>
