...
const IndicatorScreen = (props:any) => {
    ...
    AsyncStorage.getItem('UserEmail')
      .then(async user => {
        if(user) {
          ...
          if(!ValidAge) {
            GotoProfileInputScreen("AgeSelectScreen", UserEmail)
            return
          }
          

          const ValidProfileImageUrl = UserData?.hasOwnProperty('ProfileImageUrl')
          if(!ValidProfileImageUrl){
            GotoProfileInputScreen("ProfileImageSelectScreen", UserEmail)
            return
          }

          if(ValidNickName && ValidMbti && ValidGender && ValidAge && ValidProfileImageUrl) {
            setCurrentUser(UserData);
            SBConnect(SendBird, UserEmail, UserData?.NickName, UserData?.ProfileImageUrl)
          }
  ...
  const GotoProfileInputScreen = (ScreenName:string, UserEmail:string) => {
